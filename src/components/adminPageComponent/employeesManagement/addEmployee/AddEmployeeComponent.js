...
const AddEmployeeComponent = () => {
    const [createEmployee, setCreateEmployee] = useState({
        username: '', firstName: '', lastName: '', email: '', password: '', confirmPassword: '', address: '', gender: '',
        birthDate: '', nationalId: '', phone: '', startDate: '', role: '', imgPath: ''
    });
    ...
    const handleUpload = async (e) => {
        try {
            if (e.target.files[0]) {
                ...
                setCreateEmployee(prev => ({ ...prev, imgPath: res.data.secure_url.secure_url }));
            } else {
    ...
    const validateInput = () => {
        ...
        if (!(/^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[^a-zA-Z0-9])(?!.*\s).{8,15}$/.test(createEmployee.password)))
            newError.password = 'รหัสผ่านต้องมี 8-15 ตัวอักษรและต้องมีอักษรตัวใหญ่, ตัวเล็ก, ตัวเลข, อักขระพิเศษ อย่างน้อย 1 ตัว';
        if (!createEmployee.confirmPassword) newError.confirmPassword = 'กรุณากรอกยืนยันรหัสผ่าน';
        if (createEmployee.password !== createEmployee.confirmPassword) newError.confirmPassword = 'กรุณาระบุยืนยันรหัสผ่านให้ตรงกับรหัสผ่าน';
        ...
        if (!/^[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+@[a-zA-Z0-9-]+(?:\.[a-zA-Z0-9-]+)*$/.test(createEmployee.email)) newError.email = 'อีเมลไม่ถูกต้อง';
        if (!createEmployee.nationalId) newError.nationalId = 'กรุณากรอกเลขบัตรประชาชน';
        if (createEmployee.nationalId.length !== 13) newError.nationalId = 'เลขประจำตัวประชาชนต้องมี 13 หลัก';
        const digit = checkNationalID(createEmployee.nationalId);
        if (createEmployee.nationalId.slice(-1) !== digit) newError.nationalId = 'เลขบัตรประชาชนไม่ถูกต้อง';
        setError(newError);
    };
    ...
    const handleSubmit = async (e) => {
        try {
            ...
            const res = await axios.post('/users/register', {
                username: createEmployee.username, firstName: createEmployee.firstName, lastName: createEmployee.lastName,
                email: createEmployee.email, password: createEmployee.password, confirmPassword: createEmployee.confirmPassword,
                address: createEmployee.address, status: 'active', gender: createEmployee.gender, birthDate: createEmployee.birthDate,
                nationalId: createEmployee.nationalId, phone: createEmployee.phone, startDate: createEmployee.startDate,
                role: createEmployee.role, imgPath: createEmployee.imgPath
            });
                        ...
                        <FormControl isRequired>
                            ...
                            <Flex justifyContent="space-between">
                                <Flex flexFlow="column wrap">
                                    ...
                                    <Box mr="4">
                                        <Select name="gender" onChange={handleInputChange} placeholder="กรุณาเลือกเพศ">
